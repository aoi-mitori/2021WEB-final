        ...
        <?php
            ...
            if (isset($_POST['filter'])) {
                if ($_POST['filter'] == 0) {
                    ...
                    $sthAnka = $dbh->prepare('SELECT * from my_thread where root_thread_id = ? AND isAnka = 1');
                    $sthAnka->execute(array((int)$_GET['id']));
                    while ($row = $sthAnka->fetch(PDO::FETCH_ASSOC)) {
                        showMsg($row, $numFloor++);
                    }
                }
            } else {
